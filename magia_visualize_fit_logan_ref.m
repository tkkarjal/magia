function magia_visualize_fit_logan_ref(DVR,intercept,X,Y,k,roi_labels,results_dir)
...
for i = 1:size(X,2)
    ...
    xlabel('\int_0^t C_r(\tau) d\tau / C_t(t) (min)');
    ylabel('\int_0^t C_t(\tau) d\tau / C_t(t) (min)');
    title(roi_labels{i});
    a = annotation('textbox', [0.6 0.13 0.1 0.1], 'String',...
        sprintf('DVR = %.2f; intercept = %.2f',round(DVR(i),2),round(intercept(i),2)));
